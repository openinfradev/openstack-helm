...
if [[ -n $migration_interface ]]; then
    ...
    migration_address=$(ip a s $migration_interface | grep 'inet ' | head -1 | awk '{print $2}' | awk -F "/" '{print $1}')
fi
...
hypervisor_address=$(ip a s $hypervisor_interface | grep 'inet ' | head -1 | awk '{print $2}' | awk -F "/" '{print $1}')
